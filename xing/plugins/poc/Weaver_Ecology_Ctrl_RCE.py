...
class Plugin(BasePlugin):
    ...
    def verify(self, target):
        ...
        if "ecology_JSessionId" in cookie:
            self.logger.success("found Ecology RCE 1 {}".format(target))
            return url

        url_check = target + path.replace(".css", ".ccs")
        conn = http_req(url_check)
        if conn.status_code != 200 and len(conn.content) > 0:
            self.logger.success("found Ecology RCE 2 {}".format(target))
            return url
